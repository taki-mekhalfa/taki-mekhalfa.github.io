...
1. Read Uncommitted => Allows for dirty reads, a transaction can read data modified by another transaction that has not yet committed.
2. Read Committed => Does not allow for dirty reads but allows for non-repeatable reads, an operation in a transaction can read data modified by another transaction that has already committed before the current operation is run.
3. Repeatable Read => A transaction only sees data committed before it's started to run. It never sees uncommitted or committed data from other concurrent transactions.
4. Serializable => This level must prevent all possible anomalies, it provides a complete isolation between transactions. A developer does not have to care about anomalies as if all transactions we running serially. 
...
on allowed anomalies for each of the levels but still it conforms to the standard. It also uses Read Committed as the **default isolation level**.
